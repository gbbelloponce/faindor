...
- Middleware (`src/middleware.ts`) handles route protection and locale detection server-side.
...
- Locale detected in middleware via Accept-Language header, persisted in cookie.
...
## Commands
- **Dev server:** `bun run dev` (Next.js + Turbopack)
- **Build:** `bun run build`
...
- Next.js 15.3.4 and React 19.1.0 have critical security vulnerabilities (patch ASAP)
